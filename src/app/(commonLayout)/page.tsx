    ...
    <div className="container mx-auto px-4 py-6 font-sans text-gray-900">
        ...
        <div className="ml-4 flex-1 overflow-hidden">
          {latestBreakingNews ? (
            <Link href={`/news/${latestBreakingNews.slug}`} className="block">
              <div className="text-sm font-medium tracking-wide whitespace-nowrap overflow-hidden text-ellipsis">
      ...
      <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
        ...
        <aside className="md:col-span-3 order-2 md:order-1">
          ...
          <div className="space-y-10 md:space-y-8">
            {pagedLatestNews.map((news) => (
              <Link
                href={`/news/${news.slug}`}
                key={news.id}
                className="block group border-b border-gray-50 pb-5 last:border-0"
              >
                ...
                <h3 className="text-base md:text-sm font-semibold group-hover:text-red-600 transition-colors line-clamp-2 leading-snug tracking-wide">
                  {news.title}
                </h3>
              </Link>
            ))}
          ...
          <div className="mt-8 flex items-center justify-between pt-4 border-t border-gray-100">
            <button
              ...
              className="p-1 disabled:opacity-10 transition-all hover:bg-gray-100 rounded-full"
            >
              <ChevronLeft size={18} />
            </button>
            ...
            <button
              onClick={() => setLatestPage((p) => Math.min(latestTotal, p + 1))}
              disabled={latestPage === latestTotal}
              className="p-1 disabled:opacity-10 transition-all hover:bg-gray-100 rounded-full"
            >
        ...
        <main className="md:col-span-6 order-1 md:order-2">
          ...
          {pagedTopStories.length > 0 && (
            <div className="space-y-12">
              <Link
                href={`/news/${pagedTopStories[0].slug}`}
              ...
              >
                ...
                <h1 className="text-2xl md:text-3xl font-semibold group-hover:text-red-600 leading-tight mb-3 tracking-wide transition-colors">
                  {pagedTopStories[0].title}
                </h1>
                <p className="text-gray-500 text-sm line-clamp-3 leading-relaxed tracking-wide">
                  {pagedTopStories[0].summary ||
                    pagedTopStories[0].content?.substring(0, 160)}
                </p>
              </Link>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 border-t border-gray-100 pt-8">
                {pagedTopStories.slice(1, 3).map((news) => (
                  ...
                  >
                    ...
                    <h3 className="text-sm md:text-base font-semibold group-hover:text-red-600 transition-colors line-clamp-2 leading-snug tracking-wide">
                      {news.title}
                    </h3>
                  </Link>
                ))}
          ...
          <div className="mt-12 flex items-center justify-center gap-6 pt-6 border-t border-gray-100">
            ...
            <button
              onClick={() => setTopPage((p) => Math.min(topTotal, p + 1))}
              disabled={topPage === topTotal}
              className="p-1 disabled:opacity-10 transition-all hover:bg-gray-100 rounded-full"
            >
        ...
        <aside className="md:col-span-3 order-3">
              ...
              >
                ...
                <div className="flex gap-4">
                  <span className="text-xl font-semibold text-gray-200 group-hover:text-red-600 transition-colors leading-none">
                    {String(
                      (mostReadPage - 1) * itemsPerPage + index + 1,
                    ).padStart(2, "0")}
                  </span>
                  <div className="w-full">
                    <h3 className="text-sm font-semibold text-gray-900 group-hover:text-red-600 transition-colors line-clamp-2 leading-tight tracking-wide">
                      {news.title}
                    </h3>
          ...
          <div className="mt-8 flex items-center justify-between pt-4 border-t border-gray-100">
            <button
              onClick={() => setMostReadPage((p) => Math.max(1, p - 1))}
              disabled={mostReadPage === 1}
              className="p-1 disabled:opacity-10 transition-all hover:bg-gray-100 rounded-full"
            >
              <ChevronLeft size={18} />
            </button>
            <span className="text-[10px] font-semibold tracking-widest uppercase text-gray-400">
              {mostReadPage} / {mostReadTotal}
            </span>
            <button
              onClick={() =>
                setMostReadPage((p) => Math.min(mostReadTotal, p + 1))
              }
              disabled={mostReadPage === mostReadTotal}
              className="p-1 disabled:opacity-10 transition-all hover:bg-gray-100 rounded-full"
            >
              <ChevronRight size={18} />
            </button>
